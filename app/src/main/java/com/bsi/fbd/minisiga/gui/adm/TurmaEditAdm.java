...
public class TurmaEditAdm extends AppCompatActivity {
    ...
    private Turma turma;
    private int codigoAntigo = -1;
    private int posicaoCurso = -1;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_turma_edit_adm);
        ...
        anoLayout.getEditText().setText(turma.getAno());
                            ...
                            if (tipo.equals("curso")) {
                                ...
                                if (qtd > 0) {
                                    for (int i = 0; i < qtd; i++) {
                                        ...
                                        if (curso.getCodigo() == turma.getCodigoCurso()){
    ...
    public void salvar(View view) {
        ...
        if (valido){
            Response response = new Response("editturma.php",getApplicationContext());
            paramsR.put("ano",anoLayout.getEditText().getText().toString());
            paramsR.put("codigo", codigoLayout.getEditText().getText().toString());
            paramsR.put("codigo_bloco", String.valueOf(turma.getCodigoBloco()));
            paramsR.put("codigo_curso", String.valueOf(resultado.get(radioGroup.getCheckedRadioButtonId()).getCodigo()));
            paramsR.put("numero_sala", String.valueOf(turma.getNumeroSala()));
            paramsR.put("sigla_faculdade", turma.getSiglaFaculdade());
